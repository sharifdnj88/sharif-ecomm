                ...
                <div class="col-lg-8">
                    <div class="card">
                        <div class="card-header d-flex justify-content-between">
                            ...
                            <a href="{{route('admin.trash')}}" class="font-weight-bold btn btn-danger">Trash Users <i class="fa fa-arrow-right"></i></a>
                        </div>
                        <div class="card-body">
                                ...
                                <table class="table mb-0 text-center data-table-said table-bordered table-secondary">
                                    ...
                                    <tbody>
                                        @forelse ($all_admin as $item)
                                            @if ( $item -> mobile !== '01889045088' )
                                                <tr>
                                                    <td>{{$loop-> index  + 1}}</td>
                                                    <td>{{ $item -> name }}</td>
                                                    <td>
                                                        @if( isset( $item -> role -> name) ) 
                                                            {{ $item -> role -> name}}
                                                        @else
                                                            No Role
                                                        @endif
                                                    ...
                                                    <td>
                                                        ...
                                                        @else 
                                                            <img style="width: 60px;height:60px;border-radius:25%;box-shadow:0px 0px 10px 0px rgba(0,0,0,0.5)" src="{{url('storage/admins/'.$item->photo)}}" alt="">
                                                        @endif                                                  
                                                    </td>
                                                    ...
                                                    <td>
                                                        @if ( $item -> status )
                                                            ...
                                                            <a href="{{route('admin.status', $item -> id)}}" class="text-danger"> <i class="fa fa-times-rectangle-o"></i> </a>
                                                        @else
                                                        <span class="badge badge-danger">Blocked User</span>
                                                        <a href="{{route('admin.status', $item -> id)}}" class="text-success"> <i class="fa fa-check-square-o"></i> </a>
                                                        @endif
                                                    </td>
                                                    <td>
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <form action="{{route('admin-user.update', $edit -> id)}}" method="POST">
                                ...
                                <div class="form-group">
                                    <label>Mobile</label>
                                    <input name="mobile" value="{{ $edit -> mobile }}" type="text" class="form-control">
                                </div>
                                <div class="form-group">
                                    <label>Role</label>
                                    <select name="role"  class="form-control">
                                        <option value="">-select-</option>
                                        @foreach ($roles as $role)
                                            <option  value="{{ $role -> id }}" @if( $role -> id == $edit -> role_id ) selected  @endif>{{ $role -> name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="text-right">
                                    <a class="btn btn-warning" href="{{route('admin-user.index')}}">Back</a>
                                    <button type="submit" class="btn btn-primary">Submit</button>
                                </div>
                            </form>
                        </div>
